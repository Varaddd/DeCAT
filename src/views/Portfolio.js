...
const Portfolio = () => {
  ...
  const [new_contract, setNewContract] = useState();
  const [totalmints, setMints] = useState(0);
  const [new_signer, setSigner] = useState();
  ...
  const getNFT = async(event) => {
    ...
    console.log(new_contract, new_signer);
    if(new_contract != undefined && new_signer!=undefined){
        const address = document.querySelector('#walletaddress').value;
        const contractwithsigner = new_contract.connect(new_signer);
        const resp = await contractwithsigner.student_certificates(address);
